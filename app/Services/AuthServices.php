...
class AuthServices
{
     ...
     *
     * @return object
     * @throws CustomException|ServerErrorException
     */
    public function attemptLogin() : object
    {
        ...
        if( $validator->fails() ) {
            throw new CustomException($validator->errors()->first()); // 로그인 실패.
        }

        if(!Auth::attempt(['email' => $request->input('email'), 'password' => $request->input('password')])) {
            throw new CustomException(__('default.login.password_fail')); // 비밀번호 실패.
        }
    ...
    public function attemptLoginRefreshToken() : object
    {
        ...
        if(empty($refresh_token)) {
            throw new ClientErrorException(__('default.login.refresh_token_not_fount'));
        }
        ...
        if(isset($tokenRequestResult->message) && $tokenRequestResult->message) {
            throw new CustomException(__('default.login.refresh_token_fail'));
        }
    ...
    public function publishNewToken() : object
    {
        ...
        if(isset($tokenRequestResult->error_message) && $tokenRequestResult->error_message) {
            throw new CustomException($tokenRequestResult->error_message);
        }
