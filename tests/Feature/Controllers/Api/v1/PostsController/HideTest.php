...
class HideTest extends TestCase
{
    ...
    // 퍼블리시 테스트.
    public function test_post_hide_로그인_하지_않은_상태에서_요청할때()
    {
        $randPost = \App\Models\Posts::select("post_uuid")->inRandomOrder()->first();
        $testPostUuid = $randPost->post_uuid;
        $response = $this->withHeaders($this->getTestApiHeaders())->json('PUT', "/api/v1/post/${testPostUuid}/hide");
        // $response->dump();
    ...
    public function test_post_hide_존재_하지않은_요청_할때()
    {
        $randPost = \App\Models\Posts::select("post_uuid")->inRandomOrder()->first();
        $testPostUuid = $randPost->post_uuid;
        $response = $this->withHeaders($this->getTestAccessTokenHeader())->json('PUT', "/api/v1/post/1111111111111${testPostUuid}/hide");
        // $response->dump();
    ...
    public function test_post_hide_등록자와_다를때()
    {
        $randPost = \App\Models\Posts::select("post_uuid")->inRandomOrder()->first();
        $testPostUuid = $randPost->post_uuid;
        $response = $this->withHeaders($this->getTestGuestAccessTokenHeader())->json('PUT', "/api/v1/post/${testPostUuid}/hide", []);
        // $response->dump();
    ...
    public function test_post_hide_정상처리()
    {
        ...
        $response = $this->withHeaders($this->getTestAccessTokenHeader())->json('PUT', "/api/v1/post/${testPostUuid}/hide", []);
        // $response->dump();
