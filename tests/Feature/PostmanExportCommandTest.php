...
class PostmanExportCommandTest extends TestCase
{
    ...
    public function test_export_command_supports_custom_output_and_generates_expected_requests(): void
    {
        ...
        $this->assertSame('새벽 배포에서도 흔들리지 않는 Laravel API 설계', $this->decodeRawBody($postCreate['request']['body'])['title']);
        $this->assertSame(['샘플태그'], $this->decodeRawBody($postCreate['request']['body'])['tags']);
        $this->assertSame(
            "# 배포 직전, 다시 보는 API 기본기\n\n사용자는 빠른 응답을 원하지만 운영 환경은 늘 예측대로 움직이지 않습니다. 그래서 이번 글에서는 Laravel API를 설계할 때 끝까지 지켜야 할 세 가지 기준을 정리합니다.\n\n## 핵심 원칙\n- 응답 포맷은 처음부터 일관되게 유지합니다.\n- 예외 메시지는 사용자와 개발자 모두가 이해할 수 있어야 합니다.\n- 서비스 로직은 컨트롤러 밖으로 분리해 테스트 가능성을 확보합니다.\n\n## 마무리\n좋은 API는 기능이 많은 API가 아니라, 오래 운영해도 설명 가능한 API입니다.",
            $this->decodeRawBody($postCreate['request']['body'])['body']
        );
