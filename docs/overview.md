...
- `POST /api/v1/posts/{uuid}/images` : 게시글 이미지 업로드 (인증 필요, `purpose=body|cover`)
- `POST /api/v1/posts/{uuid}/cover-image` : 게시글 대표 이미지 지정 (인증 필요)
...
- `purpose`: `body` 또는 `cover`

대표 이미지 지정:

```http
POST /api/v1/posts/{uuid}/cover-image
```

```json
{
  "image_uuid": "6b75a55d-9c55-48e0-846c-4b15f614f0b8"
}
```
