...
- 메타 API는 `/posts/{slug}` 같은 공개 게시글 프런트 경로만 지원합니다.
- `url`, `resolved_url`, `canonical_url`은 도메인 없는 프런트 내부 경로를 반환하고, `image_url`은 크롤러가 바로 접근 가능한 절대 이미지 URL을 반환합니다.
- `https://...`, `//...` 같은 외부 URL 입력은 거부합니다.
- 메타 API는 공개 게시글 원본 페이지 기준으로 `robots.index=true`를 반환합니다.
...
- 업로드 시 본문 표시용 WebP 이미지(`body_image`)를 `posts/{postUuid}/body-resized/{imageUuid}.webp` 경로에 함께 생성합니다. 기본 최대 너비는 `800px`, 품질은 `75`이며, `POST_BODY_IMAGE_MAX_WIDTH`, `POST_BODY_IMAGE_QUALITY`로 조정할 수 있습니다.
