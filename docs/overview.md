...
- `GET /api/v1/meta?url=/posts/{slug}` : 프런트 경로 기준 공개 페이지 메타데이터 조회 (인증 불필요)
- `GET /api/v1/meta?url=/s/{code}` : 단축 URL을 원본 경로로 해석한 메타데이터 조회 (인증 불필요)
- `GET /api/v1/short-urls/{code}` : 단축 URL 코드 조회 (인증 불필요)
- `POST /api/v1/short-urls` : 프런트 내부 경로를 `/s/{code}` 단축 URL로 생성 (인증 필요)
...
- 모든 `/api/*` 요청은 `Client-Type` 헤더가 필요하며, 값은 `config/codes.php`의 `client.type` 활성 `code`여야 합니다. `CommonCodeSeeder`도 같은 config를 DB에 반영합니다.
- 단축 URL은 `/s/{code}` 형식으로 고정합니다.
- `short_url`, `original_url`, `url`, `resolved_url`, `canonical_url`, `image_url`은 도메인 없는 경로를 반환합니다.
- `original_url`은 `/posts/{slug}` 같은 프런트 내부 경로만 허용하며, `https://...`, `//...`, `/s/...` 입력은 거부합니다.
- 동일한 `original_url`은 기존 단축 URL 코드를 재사용합니다.
- 메타 API는 원본 페이지 요청 시 `robots.index=true`, 단축 URL 요청 시 `robots.index=false`를 반환합니다.
